...
| 11 | vivo V2551A 真机（Android 16 / OriginOS，2026-08-30 无线 ADB 实测） | ✅ 全部通过：0.52/789 单进程；FGS specialUse 运行；通知权限+电池白名单已授；前台 JS 通道通知（声音+呼吸灯）；JS 死亡→原生接管实证（补收积压私信）；`am crash` 崩溃后约 2 秒进程复活，**修复后**（v0.53 起）服务新进程即清零旧心跳、原生立即接管——早期版本存在最长 90 秒通知空窗（旧心跳有效期），击杀后 25 秒发的消息曾漏收，已修复；**重启后 2 秒原生 WS 连接认证、未开 App 收到通知（channel=messages）——BootReceiver 未被 OriginOS 自启动管理拦截**；用户上滑清除后台零影响（进程未死）。注：RN 不会无 UI 自启 JS（此前的"无 UI 自启"结论系用户恰好手动开 App 造成的误判，JS 只随 Activity 启动） |
